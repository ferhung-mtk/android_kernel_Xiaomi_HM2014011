...
static int LTR553_init_flag = 0;// 0<== OK, -1 <==> fail
...
#define WT_DO_ALS_SMOOTH
#if defined(WT_DO_ALS_SMOOTH)
#define ALS_COUNT_FOR_SMOOTH  32
...
static int ltr553_enable_ps(struct i2c_client *client, bool enable)
{
	...
	else if (enable == true)
	{
		...
		APS_ERR("ltr553_enable_ps, APS_RW_PS_CONTR: 0x%x!\n", regdata);
	...
	else if(enable == false)
	{
		...
		APS_ERR("ltr553_disable_ps, APS_RW_PS_CONTR: 0x%x!\n", regdata);
...
static void ltr553_eint_work(struct work_struct *work)	
{
	...
	//APS_ERR("ltr553_eint_work obj->pending_intr =%d, obj:%p\n",obj->pending_intr,obj);

	if ((1<<CMC_BIT_ALS) & obj->pending_intr) {
		// get raw data
		APS_ERR("ltr553 als INT\n");
	...
	if ((1 << CMC_BIT_PS) & obj->pending_intr) {
		// get raw data
		APS_ERR("ltr553_eint_work, ltr553 ps INT\n");
...
static int ltr553_get_als_value(struct ltr553_priv *obj, int als)
{
	...
	if(!invalid)
	{
		...
		//after resume
		if(als_sleep_flag == 2 && als_count >= ALS_COUNT_FOR_SMOOTH/8)
		{
			als_count = 0;
			current_als_value = raw_data;
			als_sleep_flag = 0;
		...
		case SENSOR_GET_DATA:
			if ((buff_out == NULL) || (size_out< sizeof(hwm_sensor_data))) {
				...
				if (err = ltr553_read_data_ps(obj->client, &obj->ps)) {
					...
					sensor_data->value_divide = 1;
					sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
					APS_ERR("ltr553 get ps raw_data = %d, sensor_data =%d\n",obj->ps, sensor_data->values[0]);
				}
			}
			break;
		default:
...
static int ltr553_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(err = ltr553_create_attr(&ltr553_i2c_driver.driver))
	{
...
static int LTR553_local_init(void)
{
	struct acc_hw *hw = ltr553_get_cust_alsps_hw();
	APS_FUN(); 
